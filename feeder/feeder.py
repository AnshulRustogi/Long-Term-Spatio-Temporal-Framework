    ...
    def __getitem__(self, index):
        ...
        if self.down_sample:
            if length<=60:
                ...
                #np.ceil(x): an smallest integer i such that i>=x
                #np.random.uniform(low=a, high=b, size=int): a random number z such that a<=z<b 
                #rs is some random integer between 0 and [(length-10)/50]
                rs = int(np.random.uniform(low=0, high=np.ceil((length-10)/50)))

                #np.ceil(x): an largest integer i such that i<=x;
        ...
        #if length<=60, then input_data_dnsp.shape 3, 50, 25, 2
        #else input_data_dnsp.shape = 3, 50, 25, 2
        #input_data.shape = (3, 300-10, 25, 2) all the frames except for the last 10 frame
        #data_last.shape = (3, 1, 25, 2) last 11th frame
        #target_data.shape = (3, 10, 25, 2) last 10 frames
        #label is integer
