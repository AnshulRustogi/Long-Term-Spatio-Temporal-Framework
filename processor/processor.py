...
class Processor(IO):
    ...
    def __init__(self, argv=None):
        #Loading arguments from config file
        #Function load_arg location in IO file
        self.load_arg(argv)

        #Loading function from below
        #Creating 4 dicitonaries to store the model information, result information, epoch information and meta information
        self.init_environment()

        #Loading function from recognition.py
        #Loading both the models and also the SGD optimiser
        self.load_model()

        #loading function from io.py
        #No weights are presenr
        self.load_weights()

        #loading function from io.py
        #transfers the model to the development platform. In our case to GPU
        self.gpu()
    ...
    def load_data(self):
        ...
        #feeder class is at feeder.feeder.Feeder
        """
        ...
        if self.arg.phase == 'train':
            ...
            for epoch in range(self.arg.start_epoch, self.arg.num_epoch):
                ...
                if epoch < 20:
                    ...
                    self.train(training_A=True, epochNo=epoch)
                    self.io.print_log('Done.')
                else:
                    self.io.print_log('Training epoch: {}'.format(epoch))
                    self.train(training_A=False, epochNo=epoch)
                    self.io.print_log('Done.')
